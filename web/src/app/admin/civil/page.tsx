...
export default function AdminCivilPage() {
  const [list, setList] = useState<CivilRow[]>([]);
  ...
  const deleteOne = (id: number, title: string | null) => {
    ...
    fetch(`/api/admin/civil/${id}`, { method: "DELETE" })
      .then((r) => r.json())
      .then((d) => {
        if (d.code === 0) load();
      });
  ...
  return (
    <div>
      <h1 className="text-xl font-semibold text-slate-800">事业编岗位管理</h1>
                ...
                {list.map((row) => (
                  <tr key={row.id} className="border-b border-slate-100">
                    ...
                    <Td>
                      ...
                      <button
                        type="button"
                        onClick={() => deleteOne(row.id, row.title)}
